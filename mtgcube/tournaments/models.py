...
def user_directory_path(instance, filename: str):
    ...
    fname = f"{'checkin' if instance.checkin else 'checkout'}-{now.strftime('%H%M%S')}.{f_extension}"
    tstring = str(instance.draft.phase.tournament).replace(" ", "-")
    dstring = (
        f"phase-{instance.draft.phase.phase_idx}-{instance.draft.cube.name}".replace(
            " ", "-"
        )
    )
    return f"images/userupload/{tstring}/{instance.user.username}/{dstring}/{fname}".lower()
...
class Image(models.Model):
    ...
    def __str__(self):
        ...
        draft = str(self.draft) if self.draft else ""
        return f"{self.user} - {draft} - Draft ID {self.draft_idx} - {'Checkin' if self.checkin else 'Checkout'} - ({img_time_fstring})"
